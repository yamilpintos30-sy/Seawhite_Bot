...
function welcome(displayName?: string): string {
  return displayName
    ? `¡Hola, ${displayName}! 👋 Soy el asistente virtual de *SEA WHITE S.A.*`
    : "¡Hola! 👋 Soy el asistente virtual de *SEA WHITE S.A.*";
}
...
export function createMenuHandler(menu: Menu, options: { welcome?: boolean } = {}): StateHandler {
  return {
    ...
    enter(ctx: HandlerContext): string[] {
      ...
      return options.welcome ? [`${welcome(ctx.session.contact?.displayName)}\n\n${text}`] : [text];
    },
